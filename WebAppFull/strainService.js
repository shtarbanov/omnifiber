...
async function initStrainService(){
  try{
    ...
    chrStrain.addEventListener('characteristicvaluechanged', event => { //an event is returned
      ...
      a5dv=(toVoltage(a5)-a5V0).toFixed(2);
      a3dv=(toVoltage(a3)-a3V0).toFixed(2);
      a2dv=(toVoltage(a2)-a2V0).toFixed(2);
      a1dv=(toVoltage(a1)-a1V0).toFixed(2);

      a5dl=to_dl(a5dv).toFixed(2);
      a3dl=to_dl(a3dv).toFixed(2);
      a2dl=to_dl(a2dv).toFixed(2);
      a1dl=to_dl(a1dv).toFixed(2);

      document.querySelector('#strainVal').innerHTML = 'pin.29.(A5): ' + a5 + '<br>pin.05.(A3): ' + a3 + '<br>pin.04.(A2): ' + a2 + '<br>pin.03.(A1): ' + a1
      document.querySelector('#voltageVal').innerHTML = 'A5: ' + toVoltage(a5) + '<br>A3: ' + toVoltage(a3) + '<br>A2: ' + toVoltage(a2) + '<br>A1: ' + toVoltage(a1)
